...
export function IdeaForm({ initialData }: IdeaFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    coreConcept: initialData?.coreConcept || '',
    problemToSolve: initialData?.problemToSolve || '',
    stage: initialData?.stage || ('idea' as Stage),
    primaryNextStep: initialData?.primaryNextStep || '',
  })
  ...
  return (
    <div className="animate-fade-in mx-auto max-w-2xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={isEditing ? `/ideas/${initialData?.id}` : '/ideas'}>
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            {isEditing ? 'Refine your concept' : 'Capture it once, clearly'}
          </p>
        </div>
      </div>
      ...
      <GlassCard>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 pt-4">
            <Link
              href={isEditing ? `/ideas/${initialData?.id}` : '/ideas'}
